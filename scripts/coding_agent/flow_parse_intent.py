...
def _extract_list_items(text: str, header: str) -> List[str]:
    """Extract list items from markdown"""
    import re

    if header:
        # Find section after header
...
def _extract_key_value_pairs(text: str, header: str) -> Dict[str, str]:
    ...
    if match:
        ...
        pair_matches = re.findall(r'  - (.+?):\s*(.+?)(?:\n|$)', pairs_text)
        pairs = {key.strip(): value.strip() for key, value in pair_matches}

    return pairs


def _extract_folder_structure(text: str) -> Dict[str, str]:
    """Extract folder structure from markdown code blocks"""
    import re

    structure = {}
    # Look for code blocks with folder structure
